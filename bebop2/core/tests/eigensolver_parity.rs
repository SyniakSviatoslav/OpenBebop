...
//!
//! bebop2 currently has THREE independent eigensolvers with NO parity gate between them:
...
//! precision regression, a wrong sign), the others will keep reporting the old answer and
...
//! three solvers agree on a fixed set of SYMMETRIC reference matrices (so "all real
...
fn eigensolver_triple_parity() {
    ...
    for r in references() {
        ...
        worst = worst.max(d_jg).max(d_jk);
        println!(
            "[PARITY-GATE] {} (n={}): |Δλ(jacobi,general)|={:.2e}  |Δλ(jacobi,kalman)|={:.2e}",
            r.name, r.n, d_jg, d_jk
        );
    }
