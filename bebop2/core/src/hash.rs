...
mod tests {
    ...
    fn sha512_kat_green() {
        for (input_hex, expected) in kat::vectors::SHA512 {
            ...
            assert_eq!(got.as_slice(), dehex(expected).as_slice(), "SHA-512 KAT failed for {input_hex}");
        }
    }
    ...
    fn sha3_256_kat_green() {
        for (input_hex, expected) in kat::vectors::SHA3_256 {
            ...
            assert_eq!(got.as_slice(), dehex(expected).as_slice(), "SHA3-256 KAT failed for {input_hex}");
        }
    }
