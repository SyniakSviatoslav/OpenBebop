...
- **B4** OPEN: `aws-lc-rs`/`ring` enter via `quinn`+`rustls` (`rustls-platform-verifier`). Either accept + document, or move QUIC/TLS behind a feature so the default tree is genuinely from-scratch. Extend `deny.toml`.
